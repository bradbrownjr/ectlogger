...
- **🪪 Operator Profiles** — Click any callsign in chat or the check-in list to see who it is: name, check-in stats, most-attended nets, a QRZ.com link, and an optional personal website or YouTube channel link — works even for a check-in with no linked account
...
- **📋 Check In as Logger** — A net's owner or active co-manager/rotation member can check themselves in as Logger, not just NCS, so someone can open a lobby and start logging stations while waiting for the scheduled NCS
...
- **🔐 Authenticated Nets** — Optional per-net/schedule toggle letting NCS or Logger confirm a checked-in station's identity by comparing the code on their authenticator app to the one ECTLogger computes for that account, shown as a padlock on the check-in row. Requires the station to have an ECT Logger account with two-factor authentication (MFA) set up
- **✨ Sneak-In Arrival Alert** — When a station checks itself in without staff typing it in, its row briefly flashes gold, with a small arrow pointing the way if it landed off-screen, so NCS/Logger notice new arrivals even while running the net mostly by voice
...
- **🏷️ Drill and Demo labels** — Mark traffic as Drill (a full exercise, still logged and reported like real traffic) or Demo (throwaway test data, excluded from reminders and reports, deletable by its creator or an admin at any time), at filing time or later
