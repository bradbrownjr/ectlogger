...
import { useCreateScheduleContext, Frequency } from '../../contexts/CreateScheduleContext';

// ========== TAB 3: COMMUNICATION PLAN ==========
// Frequency / channel selection table — check boxes to include in the schedule
...
const CommunicationPlanTab: React.FC = () => {
  ...
  const filteredFrequencies = frequencies.filter((freq: Frequency) => {
  ...
  const sortedFrequencies = [...filteredFrequencies].sort((a: Frequency, b: Frequency) => {
    let aVal = '';
    let bVal = '';
  ...
  const doSaveEdit = async (overrideForm?: Frequency | null) => {
