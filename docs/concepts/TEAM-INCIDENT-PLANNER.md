...
**That refusal is about containers that own staffing, and it stands. An incident record that owns no people is a different thing — and it now exists.** The statewide drill of 2026-09-17 (see [`USER-STORIES.md`](../USER-STORIES.md)) produced the evidence: a storm is several nets, more than one tag board, and dozens of messages and observations, and the net is currently the only thing that can hold any of them together. An earlier revision of this paragraph answered that with a bare reference carried by records. That was not enough, for a reason worth stating plainly: **not every incident warrants a net, so an incident has to be creatable before any record exists to carry a label.** Monitoring public-service radio during a storm, opening a shelter tag board, relaying one supply request — each is a real operation with no net anywhere in it.

So the incident is a first-class record with an open and closed state, and the refusal below is narrowed to what it was always actually about. **It holds identity and status — name, type, served agency and that agency's own reference number, times, opener, participating teams — and it owns no people.** No shift table, no assignment board, no attendance clock, no roster, no staffing role. It creates nothing and closes nothing: closing an incident closes no net and no board, exactly as closing a board closes no net today. Several teams may participate in one incident, which is what the drill actually needed and what a single-team container could never have expressed, and each team's records stay scoped to that team unless its own setting says otherwise.

The roadmap's Incident Operations Log entry holds the full design and owns the build; this module adopts the record rather than defining it. **The tripwire moves with the boundary:** the lifecycle was negotiable and has been negotiated in the open, but the moment this record grows shifts, assignments, a roster, an attendance clock, or permissions over persons rather than over records, it has become the entity refused here.
...
- Add plan versioning, approval, restricted distribution, copy/replan behavior, and actual-versus-objective review. Store template edition and minimal issued snapshots — **reusing the issued-document mechanism the roadmap's Incident Operations Log entry defines, not a second one.** A re-issued package reproduces the copy that was distributed or is visibly a revision beside it; it is never a silent second original.
