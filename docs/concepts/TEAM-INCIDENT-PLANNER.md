...
**That refusal is about containers that own staffing, and it stands. A correlation reference is a different thing.** The statewide drill of 2026-09-17 (see [`USER-STORIES.md`](../USER-STORIES.md)) produced the evidence: a storm is several nets, more than one tag board, and dozens of messages and observations, and the net is currently the only thing that can hold any of them together. The answer is an optional **incident reference that records carry** — preferring the served agency's own incident number, which section 5.14 already captures — never a parent that owns them, creates them, or closes them. The roadmap's Incident Operations Log entry holds the design and its tripwire: if that reference ever grows a lifecycle, its own permissions, its own child records, or a staffing role, it has become the entity refused here, and that has to be decided deliberately rather than reached by accretion.
