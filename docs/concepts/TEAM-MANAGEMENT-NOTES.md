...
- **Travel time is a different quantity, not a fourth source.** The three above are competing measurements of one contribution, and the adapter picks one. Round-trip travel time, which a served agency's sign-in sheet may collect because a grant match counts it (see section 5.19), measures time spent *not* at the place. It is carried alongside a duration and never folded into it, never reconciled against it, and never inferred from a home address or a named location — it is self-reported by the person who travelled.
...
| Membership stage | Interested, trainee, active, reserve, historical, **and a value meaning attended once as an external participant** | Authorized roster staff, with dated changes |
...
The external-participant stage exists because a served agency's sign-in roster (section 5.19) records everyone in the room, and a county EMA meeting contains fire chiefs, public works staff, Red Cross, and agency meteorologists who are not team members and must not become roster entries as the price of being counted present. It is not a prospect: it must be excluded from outreach, staffing search, readiness counts, and the team headcount by default, and repeated attendance is a prompt to invite someone, never an automatic promotion.
...
| Tag board and member tag | Team-owned presence occasion, and one presence record per person per board carrying state, place, optional task label, optional expected-out time, recorder, and channel. A board records **who authorized it** (name, role, and agency as free reference, since an incident commander is frequently not a platform user) separately from **who opened it** in the app, and carries a current keeper transferable by audited handover modeled on ICS transfer of command. Opening a board under external authority confers none of it, sets no alert stage, and is not activation. Who may open one is a per-team setting (default: any active member), with **at most one open board per team** so duplicates are prevented by constraint rather than by permission. **Never creates or is created by a `CheckIn`**, in either direction. Any net, alert-stage, or plan link is an optional reference, never containment, and closing either side must not close the other. No automatic tag-out exists; a board close with open tags requires explicit acknowledgment. Durations are a third canonical actual-time source that the report adapter reconciles, never sums. Optional self-reported round-trip travel time is a separate quantity carried alongside, never added to a duration and never inferred. A board may render a served agency's sign-in roster from an agency-owned template (a section 5.20 register entry, agency text carried verbatim with its revision date) and, more usefully, **export its own rows for the agency's master attendance spreadsheet** so the transcription step disappears. Export our rows only; this module never holds a multi-team attendance store. Each exported row carries a stable identity from board plus membership, and the board records what was exported and when, because a re-run otherwise appends silent duplicates to a sheet this app cannot see. |
...
- Ship member and EC/AEC guidance with each release: onboarding/assisted entry, import, privacy/delegation, readiness search, and later planning. Update README, the documentation site's audience paths, DESIGN, DEVELOPMENT, PRIVACY, ROADMAP, and CHANGELOG when corresponding behavior actually ships. Coordinate the Events concept update before implementing shared contracts.
