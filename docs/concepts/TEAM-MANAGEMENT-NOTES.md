...
| TM-47 | As an EMA coordinator, I want the paper sign-in sheet filled from records we already have. | A board renders the agency's own roster template with member details populated, travel time carried separately from presence time, external attendees included without becoming members, and every row that was not self-attested visibly marked as recorded by someone else; a blank pre-populated sheet can be printed and transcribed afterward when there is no power or network. |
...
| Served-agency sign-in roster template | None; no layout assumed | 5.14, 5.19 |
