...
**Backup NCS access.** Anyone on the schedule's Authorized Net Staff list (and any active rotation member) is granted acting-NCS access the moment they check themselves into any net from that schedule, even on a week that isn't theirs — so they can pick up net control if the scheduled NCS is unavailable or has to step away, without waiting on the owner or an admin to assign a role. When checking in, they'll see a **Check in as NCS / Check in as Participant** choice (both in the check-in prompt and the check-in form) to decide up front, and the toolbar's **Acting as NCS / Acting as Standard** control lets them change their mind at any time afterward.

**Backup Logger access.** The net's owner can likewise check themselves in as **Logger** — useful for opening a lobby and stepping in to log stations while waiting for the scheduled NCS to arrive. The same authorized net staff and rotation members eligible for NCS above can choose Logger instead if that's the more useful role that day. Unlike NCS, this doesn't require a schedule at all — it works on an ad hoc net too, since being the owner is enough on its own.
...
- **Edit your own messages** — Hover your own message and click the pencil. The message becomes an editable box in place: Enter saves, Escape cancels. The corrected wording replaces the original for everyone immediately and is marked "(edited)" next to the time, so a wrong frequency or a typo in a callsign can be fixed without deleting the message and retyping it. You can edit your own messages only, with no time limit, and net managers cannot rewrite anyone else's words (they can still delete a message).
- **Mention a station with @** — Type `@` followed by part of a callsign and pick from the list of stations checked into the net. The mentioned operator's copy of the message flashes gold when it arrives, so a question aimed at one station is not lost in a busy chat. Only stations currently checked in with an account are offered; an `@` followed by anything else is just text.
- **Reply to a message** — Hover any message and click the reply arrow. A bar above the input shows what you are answering (press Escape or the X to drop it), and the sent message carries a quote of the original above your own text. Clicking that quote jumps back to the message being answered. Replying always mentions the station you replied to, so they see the answer even if you did not type their callsign.
- **Mute a station** — Hover a message from a station you'd rather not see (spam, off-topic chatter) and click the mute icon next to Reply. Their messages stop appearing for you immediately; nobody else's view changes, and the net log is unaffected. A banner appears above chat whenever you have anyone muted, showing how many and letting you unmute from there. If you're NCS or Logger on the net, shift+click the same icon instead to mute a station for *everyone* in the chat — useful for a spammer rather than someone only you find annoying. They aren't told they've been muted; their messages simply stop reaching anyone live, though the net's stored log and any export still include everything. The manage list shows staff who muted a station and when.
...
2. On a net with one frequency (or none), check every other station the reporting station can hear on the currently selected frequency, and optionally set their operating position (Home, Field Deployed, or your own text). On a net with more than one frequency, the dialog instead shows one column per frequency so you can check off who was heard on each in a single pass, instead of switching a dropdown and re-checking the list per frequency. Save — the dialog reconciles the checked set for each frequency, so unchecking a station removes that report.
3. A **Coverage** button appears in the net toolbar, opening a **Station Coverage** panel alongside Chat, Activity Log, and the Check-In Map. It lists every reported connection, sortable by column, with one-way and two-way paths distinguished, a timestamp for when each was reported, and a filter to narrow the list to one callsign. Each connection is labeled with its amateur band (2m, 40m, etc.) when it can be determined — from the frequency selected when the report was recorded, or automatically from the net's own frequency when it only has one. A download button next to the search and map icons exports the current list to a CSV file. If a report was logged without picking a specific frequency (or the wrong one), NCS, Logger, and Relay can correct it directly from the Frequency column's dropdown; a station can correct its own past reports the same way.
...
Your Profile also gets a **Coverage** tab: a personal map of stations you've confirmed hearing from your Home operating position, aggregated across every net you've participated in, each pin labeled with when you last heard them. Pins are color-coded by band (HF, VHF, UHF) — a station heard on more than one band blends their colors on that pin, and a legend below the map explains the colors, with the full per-band breakdown (which bands, and when you last heard each) available by tapping or hovering the pin.
