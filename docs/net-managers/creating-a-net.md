...
To start an ad hoc net with no schedule behind it, use **Create Net** from the dashboard. To create one from an existing schedule, use the **Create** button on that schedule's card in the Scheduler -- it opens the same form, pre-filled with everything the schedule already knows.
...
Editing a closed or archived net still works -- a typo or a wrong setting can still be fixed -- but nothing about it re-sends the closing email or re-triggers a notification. It updates the record only.
