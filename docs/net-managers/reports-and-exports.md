...
Each recipient gets ICS-309 formatting if either the net turned it on or that recipient's own profile prefers it. A **Relay** station isn't automatically included by the role check above; if a relay operator wants their own copy of the log, have them subscribe to the schedule directly.
