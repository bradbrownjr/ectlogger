...
Every number on the page -- instance count, check-in counts, leaderboards, and the history log below -- respects one filter at the top: **30d**, **90d**, **1y**, or **All**. It defaults to 30 days. Switching it re-scopes the whole page at once, so a schedule that's been running for years doesn't drown a recent trend in old data unless you ask it to.
