...
| Net closure log | The net closes | The net's owner, every explicit subscriber to that schedule, and (since 2026-09-05) every active NCS and Logger on that specific net — so running a net you never separately subscribed to still gets you the log. Relay is meant to be on that list and currently isn't (a [known issue](/docs/about/known-issues/)) | Profile setting "Net close notifications (with log)" (on by default), and the master switch, checked per recipient |
