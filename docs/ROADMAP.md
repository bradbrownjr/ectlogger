...
**✨ Auto-open lobby before scheduled start** *(KC1JMH)* — **live on beta, pending production sign-off**  
...
An off-by-default setting that automatically transitions a net into Lobby mode without requiring the NCS to click a link. The NCS can still open the lobby manually at any time.

**Design history:** first cut (2026-07-29) covered only recurring (daily/weekly/monthly) schedules, since those are the only type with a computed start time to count down from. Field feedback on beta pushed it further: ad-hoc and one-time schedules have no such time at all, so the setting now branches by schedule type on the Schedule tab of the schedule editor:
- **Daily/Weekly/Monthly** — "Open the lobby automatically before the net" + a 15/30/60-minute offset picker. Unchanged from the original design.
- **Ad-Hoc** — "Enable lobby at start of net", no offset. Since there's no scheduled time, this is honored directly in `start_net()`: when a human clicks Start, the net stages through Lobby instead of skipping to Active. No background job involved.
- **One-Time** — adds the one thing one-time schedules never had: an optional **Net Start Time** field, sitting in the same slot Daily/Weekly/Monthly show their recurrence. Left blank, the lobby toggle below reads "Enable lobby" and behaves exactly like Ad-Hoc's manual-start branch. Filled in, the toggle becomes "Open the lobby automatically before the net" with the same 15/30/60 offset picker as a recurring schedule, and from there reuses the full recurring mechanism (background job, offset, staffing guard) unchanged. There is no separate mode selector — which behavior applies is read directly off whether the start-time field has a value. The field's value is passed as an override to `POST /templates/{id}/create-net` (`CreateNetFromTemplateRequest.scheduled_start_time`), and is independent of the lobby toggle (a one-time net can get a real start time with the lobby left off, same as any recurring net can).

*(An earlier iteration of this control asked "Open lobby: Now / At a specific time" as an explicit selector nested inside the toggle. Replaced per field feedback: with the start-time field promoted to always-visible, hovering behind a lobby-specific selector for what it means was redundant — the presence of a start time already says everything the selector said.)*

Two things this second round surfaced that the first round didn't need:
- `auto_lobby_minutes`'s lower bound was 1; changed to 0 across all four Pydantic schemas so it can serve as the "enabled, no offset" sentinel for Ad-Hoc and a One-Time net with no start time.
- The background scheduler (`_check_and_auto_open_lobbies` / `_find_lobby_candidates`) originally matched only `status == SCHEDULED`, on the assumption that only the recurring auto-create job produces nets destined for auto-lobby. That's wrong: every manually created net — including a one-time net given a specific start time via the new override — is created `DRAFT`, never `SCHEDULED`. The filter now matches both statuses. (This means the very first shipped version had a latent gap: a plain net created via Create Net with a `scheduled_start_time` and `auto_lobby_minutes` set directly would have silently never been picked up. Fixed by the same broadened filter, no separate patch needed.)
- Caught live on beta (net 52, 15-minute offset, service tick at :43 then :58 straddling an :45 open-lobby window): `NCSReminderService.CHECK_INTERVAL_MINUTES` was 15, the same as the smallest configurable offset. A poll tick landing just before the open-lobby window meant the *next* tick could land just before the net's official start, opening the lobby with almost no lead time — not a logic bug (the guards and offset math were all correct in isolation), a resolution mismatch between the poll cadence and the smallest offset choice. Reduced to 1 minute. Every check in that loop is dedup-gated (a log row, an idempotent status transition, or a re-derivable existence check) and the ±30-minute tolerances used by the reminder/auto-create checks have ample headroom at this tighter interval, so there was no reason the interval needed to be as coarse as 15 minutes in the first place.

**Design decision (2026-07-29, KC1JMH):** the setting is gated at the schedule level and overridable per net, because auto-lobby collides with the existing auto-cancel sweep. `_check_and_archive_stale_scheduled_nets` identifies a net that never ran by one signal only: it is still in SCHEDULED status 24 hours after its start time. Auto-lobby moves nets with a scheduled time out of that status on a timer, so a net nobody showed up for would sit in Lobby on the dashboard forever. The sweep now also catches a LOBBY net where `lobby_opened_automatically` is true, `started_at` is null, and there are no check-ins — regardless of which status it started from. Manually opened lobbies (a human clicked Start, whether or not a scheduled time was involved) are never swept; the scheduler must not undo a human action. The same root issue affects notifications: the single "net starting" subscriber email fires at lobby open (see `nets_core.py::start_net`), so an automated background open stays silent until an NCS checks in or the net goes ACTIVE — but a *manual* Start (Ad-Hoc, or a One-Time net with no start time) sends the notification immediately, same as any other manual start, since a human is right there confirming the net is happening.

Requires:
- Nullable `auto_lobby_minutes` column on **both** `NetTemplate` (the schedule default) and `Net` (the occurrence), following the existing `chat_grace_period_minutes` / `self_checkin_enabled` pattern. Null = disabled, `0` = enabled with no offset, so existing schedules are unaffected.
- `_get_or_create_scheduled_net` and `create_net_from_template` both copy the value forward at creation time. The scheduler reads `Net.auto_lobby_minutes` only and never re-reads the template, so a per-net "off" cannot be undone by a later sweep.
- `lobby_opened_automatically` boolean on `Net`, set only by the scheduler (never by a manual Start), so the stale sweep can distinguish an auto-opened lobby from a human-opened one.
- `start_notification_sent_at` on `Net` makes the "net starting" email idempotent, since it's now called from more call sites (manual start, go-live, NCS-arrival-in-an-auto-lobby) than the single site it originally had.
- `app/net_start.py` holds the shared logic: `lobby_open_due()` (pure offset math, unit-tested — the same class of bug as the June 2026 naive-UTC issue), `auto_open_lobby()` (the scheduler's side-effect-free lobby transition), and `send_net_start_notifications()` (the idempotent email).
- Guards on the background path: skip if the net is not DRAFT/SCHEDULED, if the occurrence has a cancelled `NCSScheduleOverride`, if no NCS is assigned for the occurrence, or if the scheduled start time has already passed. None of these apply to the manual-start path (Ad-Hoc, one-time "Now") — a human clicking Start is already the confirmation these guards exist to provide.
- Tests: `backend/tests/test_auto_lobby.py` (offset window, staffing guard, stale-sweep cases, DRAFT-vs-SCHEDULED candidate matching, manual-start forced-lobby) and `backend/tests/test_templates.py` (the `create-net` override).
...
**🔬 Verify 1-hour reminder send time against the net's scheduled start** *(W1BKW)*  
**Model:** Sonnet — the schedule-calculation path is where the June 2026 naive-UTC bug lived; write the assertion before changing anything.  
During net #44 a reminder announcing "the net was starting in 1 hour" arrived at 9:30. The duplicate-reminder half of that report was fixed in `f866587`, and the auto-create failure that skewed scheduler timing was fixed in the same commit, so this may already be resolved. Confirm against a real scheduled net that the 1-hour reminder fires within a tight window of `scheduled_start_time` minus one hour before spending effort on a fix.
