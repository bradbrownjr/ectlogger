...
**These records belong to an incident, not to a net — and that runs into something the concept docs deliberately refused.** A storm is three days, six nets, two tag boards, forty SA entries, and a dozen ICS-213s. Today the app's only container is the net, so nothing ties one operation's records together. [`TEAM-INCIDENT-PLANNER.md`](concepts/TEAM-INCIDENT-PLANNER.md) rules out a top-level `Incident` entity, and that refusal stands as written: it is aimed at a **container that owns staffing** — a second shift table, assignment board, or attendance clock — and this project does not need two of those.

What the drill actually calls for is different in kind: **a correlation reference, not a container.** The distinction is the whole design:

- It is a label records carry, never a parent that owns them. Nets, tag boards, traffic, SA entries, and activity entries each reference it optionally. Nothing is created by it, nothing is closed by it, and closing any one of them closes nothing else — the same rule [`TEAM-ACTIVATION-CALLOUTS.md`](concepts/TEAM-ACTIVATION-CALLOUTS.md) already applies between a board and a net.
...
- **The tripwire, stated now so it is not crossed by accretion:** the moment this reference grows a lifecycle, its own permissions, its own child records, or any staffing role, it has become the entity the planner refused. That may eventually be the right call, but it must be a deliberate decision with its own review, not the accumulated result of six convenient additions.
...
- What does the EMA's policy actually permit for scanner-derived content — logged internally only, shared within the county, or repeatable outward? This gates the outbound dispatcher for a whole source type and has to be asked rather than assumed.
- Where does an incident reference get created in practice, given the app is rarely the first system to know an incident exists? Most likely it arrives with the section 5.14 activation record and everything else adopts it, but confirm against a real storm rather than a drill.
