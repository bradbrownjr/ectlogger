...
**The tag board (phase M1A) is the cheapest operationally useful piece and should ship right after the roster.** The app currently has no way to record who is physically where unless a net is running, and a net check-in records a station being on the air, not a person being in a place — a non-radio volunteer or an unlicensed helper has no reason to be in a net log at all but still has to be accounted for. A tag board answers "who is where" with no net, no event, no plan, and no radio. It depends on M1 alone. See `TEAM-ACTIVATION-CALLOUTS.md` section 5.19. Its one hard invariant: **a tag never creates a check-in and a check-in never creates a tag**, and nobody is ever automatically tagged out. M1A also renders the served agency's own paper sign-in roster from the board — Cumberland County EMA's participant roster is a custom local form, not ICS-211, but its columns exist because EMPG's cost match needs name, date, times, and activity. Populating it from records the team already holds is most of the value; three things make it non-trivial, and all three are designed rather than deferred: round-trip travel time is carried alongside a presence duration and never added to it, an assisted tag is a record and not a signature so unattested rows are marked on the printed sheet, and the agency staff filling half the room are recorded as external participants without joining the roster.
