...
*Last updated: 2026-07-06 (rev 34 — Step 4 frontend page splits complete for Admin, CreateSchedule, CreateNet, Dashboard, Scheduler; NetView — all Opus-gated table extractions complete and browser-validated; sub-800 slim-down underway (useNetData, NetViewSidePanels, and the checkInActions factory extracted and browser-validated, all Sonnet-safe))*  
...
| `NetView.tsx` | 5,410 → 2,289 | **All Opus-gated extractions DONE; sub-800 slim-down in progress (Sonnet-safe).** Extracted to `components/netview/` (+ `hooks/`): `useNetWebSocket`, `CsvImportDialog`, `ArchiveDialogs`, `RoleAssignmentDialog`, `CheckInFormDialog`, `NetControlDialogs`, `NetViewHeader`, `checkInStatusHelpers`, `CheckInMobileList`, `CheckInTable` (unified desktop+detached), `useNetData`, `NetViewSidePanels`, and **`checkInActions`** (14 check-in row handlers: create/edit/delete, status+role change, the 6 inline-edit handlers, active-speaker/hand toggles, frequency claim/filter/set-active). `checkInActions` is a plain factory function (`getCheckInActions()`), not a hook — several handlers need `canManage`/`isOwner`/`ncsRoles`/`userNetRole`, computed after NetView's `if (!net) return`, and hooks can't be called after a conditional return. All `tsc --noEmit` clean; all three slim-down extractions browser-validated on beta. Not yet under ~800 lines — remaining content is ~30 net-lifecycle handlers (start/close/archive/subscribe/etc., each tied to a different already-extracted dialog) plus JSX call sites. |
