...
*Last updated: 2026-07-06 (rev 35 — Step 4 frontend page splits DONE for Admin, CreateSchedule, CreateNet, Dashboard, Scheduler, NetView (stopped short of the ~800-line target by deliberate decision — see NetView row; every risky/duplicated surface is extracted and browser-validated). Next: NCSStaffModal.tsx and Profile.tsx.)*  
...
**Step 4 — Frontend page splits** *(easiest first; NetView done, NCSStaffModal.tsx + Profile.tsx next)*  
...
| `NCSStaffModal.tsx` (1,789) | not started | **Model: Sonnet** (workhorse; Haiku-capable for the 3 smaller tabs once Sonnet does the first extraction as exemplar, same pattern as Admin.tsx). Audit 2026-07-06 confirmed this is lower-risk than NetView: no WebSocket, no focus/blur inline-edit tricks — the one moderately fiddly piece is a drag-and-drop rotation reorder (optimistic update + revert-on-error), scoped to a single tab. Plan: `hooks/useNCSStaffData.ts` (fetch + CRUD handlers, mirrors `useNetData`); `components/ncs-staff/NCSStaffRosterTab.tsx` (the largest tab, ~400 lines — has 3 near-identical staff-list variants across schedule-staff/rotation/ad-hoc-NCS views; extract a shared `StaffRosterListItem` to dedupe them, same shape as NetView's 3 check-in tables but far smaller); `NCSStaffRotationTab.tsx`; `NCSStaffScheduleTab.tsx` (+ swap/override dialog); `NCSStaffSubscribersTab.tsx`; `NCSEmailDialog.tsx`. |
| `Profile.tsx` (1,078) | not started | **Model: Sonnet** (workhorse; Haiku-capable for the Settings/Subscribers-style tabs once Sonnet does the first). Audit 2026-07-06: low risk — no WebSocket, no complex inline-edit, no attached/detached duplication. Plan: `components/profile/ProfileTab.tsx` (identity form); `AvatarCropDialog.tsx` (crop dialog + canvas helper + upload/delete, self-contained); `SettingsTab.tsx` (notification/display toggles); `ActivityTab.tsx`; `DrillDownTable.tsx` (the stat-card and favorite-nets drill-down tables are near-identical — same columns, same pagination — dedupe into one shared paginated table); `hooks/useUserStats.ts`. |
