...
- **One-Time** — "Enable lobby" with a Now/At-a-specific-time choice. "Now" behaves exactly like Ad-Hoc's manual-start branch. "At a specific time" adds the one thing one-time schedules never had — an actual start-time picker — and from there reuses the full recurring mechanism (background job, offset, staffing guard) unchanged. The picker's value is passed as an override to `POST /templates/{id}/create-net` (`CreateNetFromTemplateRequest.scheduled_start_time`), the only way a one-time net gets an official start time today.

Two things this second round surfaced that the first round didn't need:
- `auto_lobby_minutes`'s lower bound was 1; changed to 0 across all four Pydantic schemas so it can serve as the "enabled, no offset" sentinel for Ad-Hoc and one-time "Now".
