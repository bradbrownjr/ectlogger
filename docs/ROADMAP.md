...
- Toggle + number input in the schedule editor (`CreateSchedule.tsx`, Basic Info tab, grouped with the other General Net Features toggles rather than the Schedule tab, per KC1JMH 2026-07-29) for the default, and in the net editor (`CreateNet.tsx`, Basic Info) so an NCS can disable or adjust it for a single occurrence. The existing "Save to Schedule" button pushes a per-net change back up when the group wants it permanent.
