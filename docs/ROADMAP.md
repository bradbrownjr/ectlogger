...
*Last updated: 2026-07-05 (rev 31 — Step 4 frontend page splits complete for Admin, CreateSchedule, CreateNet, Dashboard, Scheduler; NetView — all Opus-gated table extractions complete (unified CheckInTable now backs both desktop-attached and detached placements); remaining sub-800 slim-down is Sonnet-safe)*  
...
| `NetView.tsx` | 5,410 → 2,860 | **All named Opus-gated extractions DONE.** Extracted to `components/netview/` (+ `hooks/`): `useNetWebSocket`, `CsvImportDialog`, `ArchiveDialogs`, `RoleAssignmentDialog`, `CheckInFormDialog`, `NetControlDialogs`, `NetViewHeader`, `checkInStatusHelpers` (shared status factory), `CheckInMobileList`, and **`CheckInTable`** — a SINGLE table component now rendered in BOTH the desktop-attached `<Grid>` and the detached `<FloatingWindow>` (a `detached` prop controls only placement chrome: mobile-hide media query, border radius, redundant detach icon). This resolved the disparity Brad flagged: the detached window was read-only and is now fully inline-editable, single-sourced with desktop so a future edit can't miss one. All `tsc --noEmit` clean. NetView is not yet under the ~800-line target, but every risky real-time/inline-edit UI surface is now extracted; the remaining slim-down (grouping handlers/state into hooks like `useCheckInActions`) is **Sonnet-safe** — the Opus gate is cleared. |
...
> **⚠️ Pending browser validation (Browserless container down since the 2026-07-05 Unraid power-loss reboot — needs Unraid app reinstall).** These are `tsc`-clean and (where a pure lift) byte-diff-verified, but NOT yet exercised in a real browser: **`NetViewHeader`**, **`CheckInMobileList`** (check at phone width), and **`CheckInTable`** in both placements. **`CheckInTable` detached mode is a behavior change** (the detached window gained inline editing) and most needs eyes-on: verify inline click-to-edit saves, status changes, active-speaker highlight, frequency-chip sub-row, and that the detached window still renders on mobile widths. Re-run the browserless smoke test (net load, dialogs open, status change, inline-edit save, WS presence, zero console errors) once the container is back, or validate manually via magic link.
