...
### Assisted Traffic Handling & Forms (Radiograms)

**✨ Assisted traffic handling — radiogram and form support for logging formal traffic on nets** *(KC1JMH)*  
**Model:** Opus to resolve the remaining design questions and lock the data model (form-definition schema vs dedicated radiogram structure is a decision that is expensive to reverse); Sonnet for each implementation phase; the auto-fill/word-count helpers are Haiku-sized once specced. The RRI/NTS Radiogram plaintext format itself is de-risked — see "Reference implementations" below — so that piece is a porting task, not a design task.

**Positioning (binding on UI copy, docs, and changelog entries):** this ships as **assisted traffic handling**, not a new message-passing medium. ECTLogger never carries a message between operators — messages always move over a real net (voice, CW, digital) or in person. What ECTLogger does is help the operator who originates, relays, or delivers a piece of traffic fill out the form correctly, remember to follow through, and keep a record — the same job a paper pad and a logbook do today. The traffic-handling community is protective of message provenance staying within recognized networks; never describe this feature as "sending," "delivering," or "routing" a message *via ECTLogger*. Describe it as logging, tracking, and reminding.

Let NCS and users log and track formal written traffic (ARRL Radiograms and other standard forms) as it's originated, relayed, and delivered through standard ham radio and in-person channels, with the paper trail stored against the net and retrievable later.

Scope:
- **Forms data model** — a `Form` table for submitted form instances (plus a form-definition / `FormField` template structure), each instance optionally linked to a `Net` so forms can also be filed outside a net. Store submitter, timestamps, form type, field values, and a **disposition** (originated / relayed / received — pending delivery / delivered) that is derived from the most recent relay/delivery log entry below, not tracked as an independent flag.
- **Traffic navbar section — single source of truth, not three** — a new top-level "Traffic" link in the navbar is the canonical home to browse/search all traffic, file a form outside a net, work the delivery inbox, and (admin) add or edit form definitions. Net, Profile, and (once shipped) Teams each need traffic to be reachable from where the operator already is, but none of them get their own separate browse/search UI — they get a lightweight, pre-filtered view that deep-links into this one central section, so nothing is duplicated and nothing has to be dug for in three different places. Concretely: NetView gets a per-net traffic tab/panel (same pattern as the embedded Chat panel) scoped to that net; Profile gets a "My Traffic" view (inbox count/badge plus a short list) scoped to that user; Teams gets a team-level rollup once it exists. The `Form` data model stays the generic/internal name (`Form`/`FormField`, matching the ICS-309-style naming already used elsewhere) — "Traffic" is the user-facing label, not a renamed table.
- **On-net entry** — NCS/users fill a form during a net and the result is stored with that net.
- **Assisted entry for operators new to traffic handling** — as the operator types the radiogram text, auto-apply the same live NTS prosign/punctuation substitution Winlink's radiogram template does (comma/period/etc. → X, question mark → INT, and so on) and auto-compute the check (word count) from it, so nobody has to memorize prosigns or hand-count words. This is exactly the popular Winlink UX and is already implemented and field-tested in the `bpq-apps` reference (`normalize_nts_text`, `count_nts_check`) cited below — port it rather than rebuilding it. Also auto-fill date/time fields.
- **Relay / delivery log** — each form instance can record one or more handoff entries describing how it moved onward: a method (digital traffic network, CW net, voice net, phone, in-person, other) plus free-text for which net/path it went out on (e.g. "Pine Tree Net", "Seagull Net") and who it was handed to (name/callsign, or "delivered to addressee"). This is the audit trail behind the disposition state — not just a delivered/not-delivered toggle — and it directly supports the HXD handling instruction (report relay and delivery chain back to the originating station) already listed in the radiogram field's HX help text.
- **Delivery inbox** — a per-user inbox of traffic that operator received over the air and has not yet logged a relay/delivery entry for, so nothing taken down on a net gets silently dropped. Each inbox item stays linked to the net it was received on, so that net's manager can see the delivery status of everything logged during their net rather than losing visibility once the net closes.
- **Delivery reminder emails** — reusing the existing SMTP path, gently remind a user holding undelivered traffic and ask them to record how/whether it was passed along (linking straight to the relay/delivery log entry, not just a yes/no), following the background-job pattern already used by `ncs_reminder_service.py`.
- **Traffic counts** — track number and list of messages handled per **user**, per **net**, and (once the Teams module ships) per **team**; surface "traffic handled" in per-net stats, global stats, and the net log / PDF report / email summary.
- **Export** — plaintext export in the field-tested RRI/NTS Radiogram format (see reference below), suitable for relaying a message over voice, packet, or any other medium; PDF export for printing, consistent with the existing ICS-309 export pattern.
- **Import** — parse a pasted/uploaded plaintext form (RRI/NTS Radiogram, ICS-213) back into a structured `Form` record, so traffic relayed in from outside ECTLogger (e.g. over packet) can be logged without re-typing every field by hand.
...
**Reference implementations — port, don't re-derive.** The adjacent `bpq-apps` repo's FORMS application (`apps/forms.py`, with form definitions `apps/forms/radiogram.frm` and `apps/forms/ics213.frm`) already implements a field-tested plaintext ARRL/NTS Radiogram format: preamble construction (number, precedence, handling, station of origin, check, place of origin, time/date), NTS punctuation/prosign normalization (`normalize_nts_text`), the check/word-count algorithm (`count_nts_check`), 5-word-per-line body formatting, and BT break placement. This format has been reviewed and approved by ARRL Digital Section Manager Jim KY2D — treat it as the authoritative RRI/NTS Radiogram text format for both the export writer and the counterpart import parser. `ics213.frm` in the same repo gives a working ICS-213 field set to start the form-definition schema from. `https://vden.org/pktnet/` has additional standard packet/form formats (e.g. a packet check-in format already ported into `bpq-apps` as `format_pktnet_checkin`) that may be worth researching for future form types beyond Radiogram/ICS-213 — not yet reviewed for ECTLogger's purposes.

`http://www.ws1sm.com/Message-Handling.html` (Wireless Society of Southern Maine — KC1JMH's own club) walks through the same four radiogram components (preamble, address, text, signature), the preamble fields A–H, the four precedence levels, and the HXA–HXG handling instructions, and links out to further primary-source material worth pulling in directly rather than re-deriving: `The-Radiogram.html` (a deeper page on the form itself), `Winlink.html`, and a `Forms/` folder with `ARRL_Radiogram_Fillable.pdf`, `ICS-213.pdf`, **`ICS-213RR.pdf`** (the reply variant — a candidate third form after Radiogram/ICS-213), and `Numbered_Radiograms_FSD_3.pdf` (ARRL's canonical ARL numbered-message list — cross-reference against `bpq-apps`'s `arl_messages.json` for accuracy/completeness before porting).

**Discrepancy to resolve before locking the text-normalization algorithm:** ws1sm.com's page states a question mark is sent as the prosign **QUERY**, but `bpq-apps`'s `normalize_nts_text` (approved by Jim KY2D) currently substitutes **INT** instead. Both can't be right for the same convention — confirm with Jim KY2D or another NTS/RRI authority which is correct (possibly a Winlink-vs-classic-NTS difference, or one source is simply dated) before the ported formatter goes live, since getting this wrong would produce radiograms that read wrong to every experienced traffic handler who receives one.

Design questions to resolve before implementation:
- Are form definitions hardcoded templates or fully admin-editable schemas? (Which forms ship first — ARRL Radiogram, then ICS-213 — is settled by the reference implementations above.)
- How is the radiogram modeled — a generic field schema covers ICS-style forms, but the radiogram preamble/address/text/signature structure (see `radiogram.frm`'s flat field list) may warrant dedicated structure instead.
- Retrieval and permissions: who can view a stored form — the submitter, the NCS of that net, admins?
- Delivery reminder cadence — how long after an item lands in a user's inbox before the first reminder, does it repeat, and does undelivered traffic ever escalate to the net's NCS/manager?
- Import strictness — how tolerant should the plaintext-import parser be of hand-relayed text (typos, a missing BT break, reflowed line breaks)? Reject on mismatch, or accept a partial parse and leave the rest for the user to fill in?
- Relay method taxonomy — "method" (digital / CW / voice / phone / in-person / other) is a fixed enum, but the net/path name ("Pine Tree Net", "Seagull Net") is local to a region and must stay free text, never a hardcoded list, since every self-hosted instance's local nets differ.
- Single hop vs. chain of custody — does the relay/delivery log record only the most recent handoff, or the full chain from receipt to final delivery? A full chain is what HXD reporting actually needs, but it's more UI to build than a single "current status" record.

This is a substantial multi-part feature: sequence the data model + Forms menu first, then on-net entry and email delivery, then the inbox/reminder loop and the stats rollup.
