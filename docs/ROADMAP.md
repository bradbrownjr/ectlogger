...
*Last updated: 2026-07-05 (rev 30 — Step 4 frontend page splits complete for Admin, CreateSchedule, CreateNet, Dashboard, Scheduler; NetView in progress — dialogs, CheckInForm, useNetWebSocket, NetViewHeader, status-helper factory, and CheckInMobileList extracted; only the unified desktop+detached CheckInTable remains)*  
...
| `NetView.tsx` | 5,410 → 3,502 | **In progress, Opus-gated.** Done: `useNetWebSocket` hook, `CsvImportDialog`, `ArchiveDialogs`, `RoleAssignmentDialog`, `CheckInFormDialog`, `NetControlDialogs`, `NetViewHeader`, `checkInStatusHelpers` (shared status icon/label/tooltip/NCS-crown factory), `CheckInMobileList` — all in `components/netview/`, wired in, `tsc --noEmit` clean. **Two Opus-gated steps remain, then back to Sonnet:** **Step 3** — extract the desktop-attached table into `CheckInTable.tsx` (pure extraction, behavior preserved; verifiable by byte-diff + tsc). **Step 4** — point the detached floating window at the same `CheckInTable`, unifying them. Highest risk: inline-edit refs/focus/blur, status handlers, NCS coloring, active-speaker highlighting, frequency-chip sub-row. **Decision (Brad):** the detached table must behave identically to desktop incl. inline editing — the current read-only detached table is a disparity Step 4 fixes; the mobile table stays intentionally simplified (already extracted). |

*Admin through Scheduler splits completed 2026-07-04. All splits verified `tsc --noEmit` 0 errors. NetView dialog cluster + form + WS hook completed and beta-validated 2026-07-05 (commits `0929758`, `485ee48`, `f3418ad`, `0fdfe31`, `556a4df`, `4e6ad87`). Header + status-helper factory + mobile-list extracted 2026-07-05 (`ec10adb`, `8b945e0`, `8c40cb3`); mobile-list transcription verified byte-identical via normalized diff. Remaining: Step 3 (desktop `CheckInTable` extraction) + Step 4 (detached unification), both Opus-gated.*

> **⚠️ Pending browser validation (Browserless container down since the 2026-07-05 Unraid power-loss reboot — needs Unraid app reinstall).** The following are `tsc`-clean and (where applicable) byte-diff-verified, but NOT yet exercised in a real browser: **`NetViewHeader`**, **`CheckInMobileList`** (check on a phone-width viewport), and the upcoming **`CheckInTable`** Steps 3–4. Step 4 in particular *changes* detached-window behavior (adds inline editing) and must be browser-tested before it is considered done. Re-run the browserless smoke test (net load, dialogs open, status change, inline edit save, WS presence, zero console errors) once the container is back.
