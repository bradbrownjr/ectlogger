...
This doesn't depend on whether the net already has an active NCS. A large exercise with several eligible staff each claiming NCS for their own desk within minutes of each other is the normal pattern, not something the system tries to prevent -- see [Frequencies and multiple net controls](/docs/net-control/frequencies-and-multi-ncs/) for what that looks like on screen. The full eligibility rules -- who counts as net staff, what a rotation member can do, and the difference between that and a schedule's co-manager tier -- are in [Roles and permissions](/docs/reference/roles-and-permissions/); this page assumes you already know whether you're eligible and covers what happens once you act on it.

If it's someone else's ad hoc net with no schedule behind it, self-grant isn't available -- they have to assign you the role directly, covered next.

## Assigning a role to someone else

Open **Roles** on the toolbar (see [The net control desk](/docs/net-control/the-net-control-desk/)). This opens **Manage NCS and Logger roles**: pick a user, pick NCS, Logger, or Relay Station, and click **Assign Role**. Every current assignment is listed below with its own remove control.

<figure>
  <img src="/docs/img/net-control/manage-roles-dialog.png"
       alt="The Manage NCS and Logger roles dialog: a user picker, a role picker offering NCS, Logger, or Relay Station, and the Assign Role button outlined in red, above a list of current assignments each with a remove control.">
...
**Stepping down** (the **Role: NCS** / **Role: Standard** toggle on the toolbar) toggles whether your existing NCS role is currently active. Step down and you genuinely lose NCS access to this net -- the backend enforces the same active flag the screen shows, so this isn't just a display change. You keep the role itself and can step back up to it later; you just aren't acting as NCS in the meantime. If you're the *only* active NCS on a net that's currently active, this toggle is blocked outright -- assign or wait for another NCS first.
...
- It clears itself the moment any active NCS becomes present again -- there's nothing to dismiss or reset by hand.
