...
- **Fix the record.** Everything in the check-in list and the role assignments stays editable after closing, so a mistake in the *log* — a wrong entry, a missing role — is still fixable even though the net itself is done.
