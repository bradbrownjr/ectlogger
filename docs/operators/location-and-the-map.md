...
Every net with at least one parseable location on a check-in has a live map, and the same map data drives the net's report and its statistics page, with one pipeline behind all three, so a station isn't plotted on one and missing from another. A few things that surprise people:

- **Checked-out stations are still plotted.** The map is a record of who took part in the net, not a live "who's still on frequency" indicator, so someone who checked in, participated, and checked out an hour ago stays on the map.
