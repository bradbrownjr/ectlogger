...
| Auto-open lobby before scheduled start | `backend/app/net_start.py`; `backend/app/ncs_reminder_service.py::_check_and_auto_open_lobbies` | `auto_lobby_minutes` on both `NetTemplate` and `Net` (null = off). Interacts with `_find_stale_nets` (an auto-opened lobby nobody attended must still be archived) and with `send_net_start_notifications` (an auto-opened lobby stays silent until an NCS arrives or the net goes live) |
