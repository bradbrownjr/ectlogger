...
| Auto-open lobby before scheduled start | `backend/app/net_start.py`; `backend/app/ncs_reminder_service.py::_check_and_auto_open_lobbies`/`_find_lobby_candidates`; `routers/nets_core.py::start_net` (ad-hoc/one-time "now" branch); `routers/templates_subscriptions.py::create_net_from_template` (one-time start-time override) | `auto_lobby_minutes` on both `NetTemplate` and `Net` (null = off, `0` = enabled with no offset). Recurring schedules use the background job; Ad-Hoc and one-time "Now" are honored directly on manual Start; one-time "At a specific time" passes `CreateNetFromTemplateRequest.scheduled_start_time` and then behaves like the recurring case. Background job matches DRAFT and SCHEDULED nets, not just SCHEDULED. Interacts with `_find_stale_nets` (an auto-opened lobby nobody attended must still be archived) and with `send_net_start_notifications` (silent for a background auto-open, sent immediately for any manual Start) |
