...
| Backup NCS auto-grant | `backend/app/routers/check_ins.py::create_check_in` (auto-grant block, after the auto-opened-lobby check) | An active co-manager (`TemplateStaff.is_co_manager`) or active `NCSRotationMember` for a net's template is auto-assigned a `NetRole("NCS")` the moment they check into any occurrence, even off-week — no separate "claim" step. Scoped to template-based nets; ad hoc nets have no co-manager/rotation pool. They can step down via the existing Acting as NCS/Standard toggle (`nets_roles.py::toggle_self_net_role`), which now actually changes backend permission thanks to the `is_active` fix above. `Claim NCS` (owner/admin-only recovery for orphaned nets) is intentionally unchanged/still narrow — redundant for this pool since they're auto-granted, but still the only recovery path on ad hoc nets |
