...
- Server-originated types (a route handler calls `manager.broadcast` after a DB write): `check_in`, `status_change`, `check_in_deleted`, `hand_raised_changed`, `net_started`, `net_lobby_opened`, `net_status_change`, `net_pause_change`, `role_change`, `can_hear_changed`, `chat_message`, `chat_message_edited`, `chat_reaction`, `chat_image`, `chat_net_mute_changed`, `identity_verified_changed`, `traffic_logged`, `traffic_log_changed`, `ping`
...
**Net actions have one rule each, shared by the endpoint and its button.**
`permissions.py` holds a pure `may_*` function per net action (edit, start,
close, import, lifecycle, claim NCS, manage roles, email subscribers) over a
`NetAccess` loaded by `net_access()` / `load_net_access()`. The endpoint calls
the rule, and `NetResponse.actions` (plus `actions_as_regular_user` for an
admin's "View as Regular User" preview) sends the same answers to the app,
where `utils/netActions.ts::getNetActions` gates each toolbar and card button.
A new net action gets a rule and a `NetActions` field; never gate its button
on the broad `canManage`. Until 2026-09-25 that is exactly what the toolbar
did, so staff saw Close/Import/Edit/Archive buttons the server refused and a
Logger could close a net with no button to do it.
...
User-facing documentation lives in the documentation site under `docs/`, organized by
**audience path**, not by feature. `docs/USER-GUIDE.md` was retired on 2026-09-19 and
replaced by it; do not recreate a single all-audiences guide.

| Path | Directory | Who it is written for |
|---|---|---|
| Get started | `docs/start/` | Somebody who has never used ECTLogger, in three tutorials |
| Operators | `docs/operators/` | Anyone checking into a net |
| Net control | `docs/net-control/` | NCS, Logger, and Relay while a net is running |
| Net managers | `docs/net-managers/` | Whoever owns a net or a recurring schedule |
| Administrators | `docs/admins/` | The admin panel and the instance's own settings |
| Reference | `docs/reference/` | Lookup tables: statuses, permissions, fields, syntax, emails |
| About | `docs/about/` | What it is, known issues, how to get help |

**When adding or changing a feature, name the paths it touches and update them:**

- **Every user-facing change updates at least one audience path.** A change nobody
  on any path would notice is not user-facing, and does not need a changelog entry
  either. If you cannot name the path, say so rather than inventing a page.
- **A roadmap item's `Docs:` line is where this gets decided**, at the moment the
  item is written rather than at the end when the budget is gone. New roadmap items
  for user-facing work carry one.
- `docs/CHANGELOG.md` — add an entry describing the change (format above).
- `README.md` — update the feature list only for genuinely new user-facing capability.
- `docs/DEVELOPMENT.md` — new technical patterns, APIs, or environment facts.
- **A page that documents an unshipped feature is a bug.** The site describes
...
- Every page carries front matter: `title`, `summary`, `kind` (Tutorial, How-to,
  Reference, or Explanation — Diátaxis), `audience`, `owner`, `revised`, `review_by`,
...
**Figures are regenerated by command, never edited by hand.** Declare the shot in
`scripts/docs-screenshots/requests/<path>.yml` and run
`scripts/docs-screenshots/run.sh --section <path>`. See that directory's `README.md`
for the field reference. Worth a full re-run after any release that changes the
check-in table, the net toolbar, or the card buttons.

**Before committing documentation, run the checker:**
...
3. **Update documentation** (the audience paths the change touches, CHANGELOG, README as appropriate — see Documentation Requirements) and run `python3 scripts/check-docs.py`
...
- **The documentation pages for its audience paths exist and are correct**, and
  `python3 scripts/check-docs.py` reports no problems. A user-facing change that
  updates no audience path is not done. See Documentation Requirements for the
  path table and the front-matter rules.
- Related documentation is updated (`docs/CHANGELOG.md`, `README.md`,
  `docs/DEVELOPMENT.md` as applicable).
...
| Automatic scheduled net creation | `backend/app/ncs_reminder_service.py::_get_or_create_scheduled_net` | Background job; catches its own exceptions and returns `None` on failure — check logs, not just HTTP responses, when debugging. **An auto-created net with no NCS assigned is intended, not a bug** — `_assign_duty_ncs` pre-assigns only when the schedule has an NCS **rotation**; with no rotation the net (and its auto-opened lobby) deliberately starts unstaffed, because clubs routinely don't know who is available until the night of, and whoever shows up claims it by checking themselves in as NCS (any active `TemplateStaff` is eligible — see the Backup NCS / Logger self-grant row). Never "fix" this by pre-assigning staff at creation time; if staff can't take an unstaffed net, the bug is in eligibility, not here. Confirmed by the user 2026-09-18 after the net 95 investigation |
...
| Auto-close net after inactivity | `backend/app/ncs_reminder_service.py::_check_and_close_inactive_nets`/`_find_auto_close_candidates`/`_last_activity_at`; `backend/app/services/net_closure.py::close_net_and_notify`/`get_last_activity_at`/`compute_auto_close_at`; `routers/nets_core.py::get_net`; `frontend/src/components/create-net/BasicInfoTab.tsx`; `frontend/src/components/create-schedule/ScheduleTab.tsx`; `frontend/src/components/netview/AutoCloseWarningBanner.tsx`; `frontend/src/hooks/useNetWebSocket.ts` | `auto_close_after_minutes` on both `NetTemplate` and `Net` (null = off, the default; 15-1440 range enforced in `schemas.py`, UI only offers 60/120/180/240). Scoped to `NetStatus.ACTIVE` only (never LOBBY). Last activity = max of the net's `started_at`, every `CheckIn.updated_at`/`checked_in_at`, and every `ChatMessage.created_at` (`get_last_activity_at`, shared by the scheduler and the endpoint below so they never disagree) -- check-ins and chat both reset the clock, so a quiet-but-monitored net is never closed out from under someone. On threshold breach, posts a custom system chat message ("Net automatically closed after N hour(s) of inactivity.") then calls `close_net_and_notify(..., post_chat_message=False)` so the closure email/ICS-309 looks exactly like a normal manual close -- the auto-close reason lives only in the chat message, never in the email. `NetResponse.auto_close_at` (only populated by `get_net`, not the list view) is `compute_auto_close_at`'s last-activity-plus-timeout preview, driving a dismissible warning banner (`AutoCloseWarningBanner.tsx`) that NetView shows once within 15 minutes of that time -- dismissal is keyed to `auto_close_at` itself (same pattern as `UpdateAvailableBanner.tsx`'s `dismissedFor`/build id) so a check-in or chat message that pushes the deadline back out and later drifts back into the window reopens it rather than staying dismissed. Since neither `check_in` nor `chat_message` WebSocket events otherwise refresh the `net` object, `useNetWebSocket.ts` takes an `autoCloseEnabled` flag (`net.auto_close_after_minutes != null`) and calls `fetchNet()` on both events only when it's true, so nets with the feature off (the default/common case) never pay for the extra fetch. History: added 2026-09-15 after net 92 (ME Dirigo Net) was left open for hours when NCS forgot to click Close; off by default since a long SKYWARN/ARES activation can legitimately go hours between check-ins. The warning banner followed the same day after net 94 ("Oops net") was opened on beta for testing and the user asked for advance warning before an auto-close fires. Tests: `backend/tests/test_net_auto_close.py` |
...
| Check-in maps (live, report, statistics) | `frontend/src/hooks/useMappedCheckIns.ts` (the one pipeline); `utils/locationParser.ts` (parse/geocode), `utils/dualMap.ts` + `dualMap.test.ts` (cluster/overview split); consumers `components/CheckInMap.tsx`, `pages/NetReport.tsx`, `pages/NetStatistics.tsx`; `backend/app/routers/geocode.py` | **Which stations get mapped is decided in the hook, never in a consumer** -- checked-out stations ARE plotted (they took part in the net; the map records participation, not who is still on frequency), and there is NO cap on addresses geocoded (Nominatim rate limiting is serialized and cached server-side). All three consumers carried their own copy of this loop until 2026-09-18 and had drifted: the report filtered out CHECKED_OUT while the live map and statistics kept them, so `/nets/95/report` plotted 10 of net 95's 11 stations while `/statistics/nets/95` plotted 11 -- the user-reported bug. That filter was written as lowercase `'checked_out'` when the report map was added (`8d327d8`) and so never matched the DB's uppercase values; `8b06a94`, a commit titled "uppercase status comparisons", silently activated a filter that had never once fired. The statistics page separately still had a 10-address geocode cap already found and removed from the other two (station 11+ vanished with no indication), and `computeDualMapData` was copy-pasted into both pages. A consumer takes `mapped`/`unmapped`/`loading` from the hook and does not pre-filter `checkIns` on the way in; `CheckInMap` passes `enabled: open` since it stays mounted while closed. **Marker appearance is likewise central**: `utils/checkInMarkers.ts` holds the status/role palette, the role split (`getMarkerRoleIds`) and `buildMarkerLegend`; `utils/checkInMarkerIcon.ts` holds the Leaflet pin (split out only because the palette must stay DOM-free to be unit-testable -- Leaflet needs `window` at import). Never hardcode a marker color or hand-write a legend. The palette is keyed to `StationStatus` (`backend/app/models.py`) and nothing else: the two per-page copies it replaced colored invented statuses (`TACTICAL`/`MONITORING`/`CHECKING_OUT` on the report, `tactical`/`priority` on the live map) and so sent real ones to a default -- the report greyed `away`/`mobile`/`announcements`/`relay`, the live map drew `checked_out` in the checked-in green, and the report's legend named two statuses the app cannot produce while omitting every color actually on the map. `getMarkerRoleIds` also fixes two bugs in the derivation NetView hand-rolled: roles come back UPPERCASE so `r.role === 'Relay'` never matched (no relay station was ever colored, and its legend entry never appeared), and an un-filtered list ignores `is_active` so a station kept the NCS color after stepping down. Marker *shape* is deliberately not unified: the report/statistics SVG-in-`img` pin survives html2canvas export, the live map's CSS-rotated square does not. See `docs/DESIGN.md` "Check-in Map Markers" and `docs/DEVELOPMENT.md` "Check-in maps". Tests: `frontend/src/utils/checkInMarkers.test.ts`, `dualMap.test.ts` |
...
| Net/schedule logo upload | `backend/app/utils.py::save_resized_logo`; `routers/nets_core.py::upload_net_logo`, `routers/templates_core.py::upload_template_logo`; `frontend/src/components/NetLogoSection.tsx` | Shared by both endpoints -- `NET_LOGO_DIR` files named `net-{id}.*`/`template-{id}.*`. Saves back in whatever format was uploaded (PNG/JPEG/WebP, all natively viewable in a browser) instead of normalizing everything to JPEG: JPEG has no alpha channel, so that used to flatten a transparent PNG onto solid black (fixed 2026-09-15). A JPEG source is still flattened to RGB (nothing to preserve); PNG/WebP keep their alpha. `save_resized_logo` reads the source format from `pil_image.format` *before* `ImageOps.exif_transpose` -- that call returns a new Image object with no `.format` when it actually rotates pixels (a portrait phone photo with an EXIF orientation tag), which would otherwise silently fall back to PNG for a real JPEG/WebP upload. The client-side crop step (`NetLogoSection.tsx::getCroppedImg`) has the same rule for the same reason: it re-encodes through a `<canvas>`, which composites transparent pixels onto black when told to export JPEG, so it now crops to PNG/WebP when the source was one, JPEG otherwise. Deleting a logo, or replacing one in a different format, globs `{net,template}-{id}.*` rather than assuming `.jpg`. Tests: `backend/tests/test_logo_upload_format.py` |
...
| Chat (messages, reactions, images, edit/@mention/reply) | `backend/app/routers/chat.py`; `frontend/src/components/Chat.tsx`; `frontend/src/api/chat.ts` | WebSocket `chat_message` and `chat_message_edited` types. `ChatMessage` columns `edited_at`, `mentioned_user_ids` (JSON-as-Text, same pattern as `CheckIn.custom_fields`), `reply_to_message_id` (self-referential, migration 067). **Edit** (`PUT /chat/nets/{net_id}/messages/{id}`) is overwrite-only (no history table) and own-messages-only with **no admin override**, unlike delete: an admin rewriting a station's words would leave that callsign credited in the net log with wording it never sent, while deleting misattributes nothing. **@mentions** are resolved server-side on send *and* on edit, only against stations checked into that net with an account, so the highlight can never point at a stranger or a guest check-in; the author is always dropped from the list. **Replies** are Signal-style single-level quotes, never threads, and always mention the quoted message's author. `ChatMessageResponse.reply_to` is a denormalized `{id, callsign, message}` preview (140 chars) so the UI needs no second lookup; it is `None` when the quoted message was deleted, which the UI renders as a reply with no quote rather than an error (SQLite does not enforce the `SET NULL`, so a dangling id is a real shape). Both the reply preview and the message text go through `redact_contact_info` for guests in `_broadcast_chat_message` -- an edit path that skipped that would reintroduce exactly what the original send redacted. Mention/reply-jump highlights reuse `sneakInHighlight.ts`'s `sneakInFade`, and fire only on live WS events, never on the initial `fetchMessages()` load (otherwise every past mention flashes at once on each reload). Roster comes from the `checkIns` prop, passed at all three placements: `NetViewSidePanels.tsx` (attached + floating) and `NetPaneWindow.tsx` (popped-out window). Tests: `backend/tests/test_chat_edit_mention_reply.py`. **Personal chat mute** (`chat_mutes` table, migration 068; `GET/POST /chat/nets/{net_id}/mutes`, `DELETE /chat/nets/{net_id}/mutes/{muted_user_id}`) hides one station's messages from the muter's own view of one net only -- client-side filtering, since the payload already reached that browser and simply isn't rendered; no other viewer and never the exported net log is affected. Scoped to authenticated viewers only (posting requires auth, so every message has a real `user_id` -- there is no guest-authored message to mute, and a guest has no account to mute from). A muted author's quoted text is also redacted inside someone else's reply preview (matched by callsign, since `reply_to` carries no `user_id`), or a quoted meme would reappear there and defeat the mute. The banner above chat shows a live count and opens a manage/unmute list -- without it a mute set once and forgotten becomes a bug report. Tests: `backend/tests/test_chat_mute.py`. **Net-wide chat mute** (`chat_net_mutes` table, migration 069; `GET/POST /chat/nets/{net_id}/net-mutes`, `DELETE /chat/nets/{net_id}/net-mutes/{muted_user_id}`) is the staff-set, server-enforced counterpart: shift+click the same mute icon (NCS/Logger only, gated by `check_net_permission(..., ["NCS", "LOGGER"])` server-side regardless of what the client sends) hides a station for *every* viewer, not just the muter. One canonical row per `(net, muted_user_id)`, not per-applier -- any active NCS/Logger can lift a mute someone else applied; doesn't persist past the net. Enforced at broadcast time, not just client-side: `_broadcast_chat_message` checks `ChatNetMute` before broadcasting and, if the author is currently muted, passes `only_for_user_ids={author_id}` to `ConnectionManager.broadcast` (`main.py:213`) so the message is echoed back to the author's own connection only and never put on the wire to anyone else -- reusing the same per-connection mechanism as the guest-PII-redaction `guest_message` path, and meaning a muted spammer's images are never even fetched by other viewers' browsers, not just hidden after arriving. `GET /chat/nets/{id}/messages` and every export still return the message unfiltered -- only the live broadcast and each client's own rendering are affected, never the stored log. It's a shadow mute: the muted station is never told. Audit trail (`applied_by_user_id`/`created_at`) is staff-visible only -- non-staff viewers see that a station is muted for everyone but not who did it. A muted author's own reply-quote redaction and message rendering exempt their own view of their own words. Applying/lifting a mute also broadcasts `chat_net_mute_changed` (no message content, safe for every connection) so an already-open tab's banner/manage list updates live instead of only on next load -- the message-level `only_for_user_ids` restriction alone would keep new messages off the wire, but wouldn't tell an already-connected viewer's UI that a mute state changed at all. Tests: `backend/tests/test_chat_net_mute.py`. |
...
| Auth (magic link + password fallback + OAuth-registered-but-unimplemented → JWT) | `backend/app/routers/auth.py`, `app/auth.py`; `backend/app/email/auth.py` | `create_magic_link_token`, `create_access_token`. Password login (`POST /auth/login`) accepts callsign or email, bcrypt-hashed, 5-attempt/15-min lockout. OAuth client registration exists but the callback exchange is a `501` stub -- not a working login path. **Email is normalized (`app/utils.py::normalize_email` -- strip + lowercase) at every schema that accepts one** (`UserBase`, `AdminUserCreate`, `MagicLinkRequest`, `ContactBase`/`ContactUpdate`) and every raw lookup uses `func.lower(User.email) ==` against an already-normalized value, so `Name@Example.com` and `name@example.com` are the same account. Before 2026-09-14, only password login's `_find_by_identifier` lowercased -- the magic-link request/verify path and admin invite did not, so re-registering with different email capitalization silently created a second account with its own check-in history (migration `072_normalize_user_emails.py` lowercases existing rows, skipping and reporting any pair that already collides) |
...
| Authenticated nets (TOTP station identity verification) | `backend/app/routers/check_ins.py` (`get_expected_code`, `verify_identity`, `_load_check_in_and_net_for_auth`), `app/auth.py::current_totp_codes`; `frontend/src/components/netview/IdentityVerifyDialog.tsx`, padlock icon in `CheckInTable.tsx`/`CheckInMobileList.tsx` | `Net.authenticated`/`NetTemplate.authenticated` (migration 070) is the per-net/schedule toggle, seeded on all net-creation paths (manual `templates_subscriptions.py::create_net_from_template` and scheduled `ncs_reminder_service.py`) same as every other template-seeded toggle. Builds on the TOTP/MFA system above but is otherwise independent: `current_totp_codes` (new -- the MFA system only ever *verified* a submitted code, never *displayed* the expected one) returns `(current, previous)` so NCS can still match a code that rolled over mid-read. Both endpoints are `check_net_permission(..., ["NCS", "LOGGER"])`-gated and 400 on a non-authenticated net; the raw secret is decrypted server-side via the existing `decrypt_mfa_secret` and never serialized into a response -- only the computed 6-digit code is. `CheckIn.identity_verified`/`identity_verified_at`/`identity_verified_by_id` record NCS's confirmation; `CheckInResponse.identity_verifiable` (computed from `user.mfa_enabled`, not stored) lets the client grey out the action for a station with no MFA rather than offering it and failing -- a check-in with no linked account (`user_id is None`) is always unverifiable, there being no secret to check. A reject explicitly clears the verified state (not just closes the dialog) so a bad match never leaves a stale confirmation. `identity_verified_changed` WebSocket broadcast keeps every viewer's padlock live, same pattern as `hand_raised_changed`. Tests: `backend/tests/test_authenticated_nets.py` |
| Email notifications (net lifecycle, reminders, logs) | `backend/app/email_service.py` (facade) + `app/email/{base,auth,net_lifecycle,reminders,net_logs,digest}.py` | `EmailService` facade class. The net-closure log email's recipient list (`services/net_closure.py::close_net_and_notify`) is the owner (if opted in) plus explicit `NetTemplateSubscription` subscribers plus — since 2026-09-05 — every active NCS/Logger/Relay on that specific net (also opted in via `email_notifications`/`notify_net_close`). Before that fix, running a net as NCS/staff did not by itself make you a recipient: a schedule's own on-duty NCS with no separate subscription row got no copy of the log for the net they just ran (reported 2026-09-04, net 84) |
| WebSocket real-time updates | `backend/app/main.py::ConnectionManager`; `frontend/src/hooks/useNetWebSocket.ts` | Server-originated: `check_in`, `status_change`, `check_in_deleted`, `hand_raised_changed`, `net_started`/`net_lobby_opened`, `net_status_change`, `net_pause_change`, `role_change`, `can_hear_changed`, `chat_message`/`chat_message_edited`/`chat_reaction`/`chat_image`/`chat_net_mute_changed`, `identity_verified_changed`, `traffic_logged`, `traffic_log_changed`, `ping`. Client-relayed: `active_speaker`, `active_frequency`, `bulk_check_in_status`. `useNetWebSocket.ts` also runs a client-side liveness watchdog (last-message timestamp + visibilitychange check) that force-reconnects a socket that looks OPEN but has gone silent past 2 heartbeat intervals — the failure mode a purely close/error-driven reconnect can't see |
...
| Permission checks (owner/admin/co-manager/staff) | `backend/app/permissions.py` | `check_net_permission`, `check_template_permission`, and the per-action `may_*` net rules behind `NetResponse.actions` (see "Net actions have one rule each" above) — used across nets/templates/check-ins routers. `check_net_permission` only honors an *active* `NetRole` (`is_active == True`) — stepping down to Standard via `toggle_self_net_role` actually revokes elevated access, not just the display |
...
| Operator profile popups (QRZ link, website, clickable callsign, guest profile) | `backend/app/routers/users.py::get_user_popup`/`get_user_popup_by_callsign`; `frontend/src/components/UserProfileDialog.tsx`, `Chat.tsx`, `netview/CheckInTable.tsx`, `netview/CheckInMobileList.tsx` | `User.website_url` (migration 071) is the optional outbound link shown on the popup alongside a QRZ.com link built from the callsign (no stored data, just a URL template). The callsign text next to the avatar is now clickable in all three call sites, not just the avatar -- except in the two check-in tables, where it's gated behind `!canManageCheckIns`: for NCS/Logger, clicking a callsign cell starts inline edit (`handleStartInlineEdit`), so the profile click would fight that; standard/view-only users get the popup instead. `GET /users/callsign/{callsign}/popup` is the guest-popup counterpart to the existing `GET /{user_id}/popup` for a check-in with no `user_id` (guest, or NCS/Logger-entered by voice) -- same public/unauthenticated shape, sourced from `Contact` (auto-populated on a callsign's first check-in) plus `CheckIn` rows matched directly by callsign instead of a User's alias set. 404s if the callsign has never checked in anywhere, same as an unknown `user_id` -- not a new enumeration surface. No `net_role` on the guest path: a callsign with no account can never hold a `NetRole` (see the Backup NCS / Logger self-grant row below). `frontend/src/utils/nameFieldGuard.ts::looksLikeUrl` extends the existing email guard to links too (a long URL in the Name field forces NCS to horizontally scroll the check-in list) -- applied at the same three sites as the email check, plus a fourth: `ProfileSetupDialog.tsx` escalates it to a hard block (`reason: 'spammy_name'`), reusing the dialog's existing global mount and disableEscapeKeyDown gate rather than adding a second mechanism, and offers the new Website field inline so the fix is one action. |
| Backup NCS / Logger self-grant | `backend/app/routers/check_ins.py::create_check_in` (grant block, after the auto-opened-lobby check); `permissions.py::is_eligible_for_ncs_auto_grant` and `is_eligible_for_logger_self_grant` (eligibility checks, one per grantable role) | Any active `TemplateStaff` member (the schedule's "Authorized Net Staff" — co-manager flag *not* required, see 2026-09-18 below) or active `NCSRotationMember` for a net's template is eligible to become `NetRole("NCS")` the moment they check *themselves* into any occurrence, even off-week — no separate "claim" step. The shared bar is `permissions.py::is_active_template_staff`; `is_co_manager` remains a strictly higher, schedule-ownership tier (ownership transfer, template merge, net archive/delete) that its own callers check separately — never re-narrow this helper to it. The net's **owner** (plus that same co-manager/rotation pool) is separately eligible to self-grant `NetRole("LOGGER")` the same way — added 2026-09-05 for the "owner opens the lobby as Logger while waiting for the scheduled NCS" workflow; unlike NCS eligibility this also works on ad hoc nets (no template needed) via the owner path. Eligibility does **not** depend on whether someone else already holds an active NCS role: a net can legitimately have many simultaneous active NCS (e.g. net 15, "GYX SKYWARN / Emergency Communications Exercise" — 8 different eligible staff each deliberately checked in as NCS within minutes of each other for their own desk). Restricted to self-check-in (`current_user.id == linked_user_id`); a staff-entered check-in (NCS/Logger logging someone in, e.g. by voice) never grants either role, always Standard — they can promote that station afterward via the check-in row's status dropdown instead (`checkInActions.ts::handleStatusChange`, already existed, no eligibility gate beyond `canManageCheckIns`). Scoped to template-based nets for the NCS path only; ad hoc nets have no co-manager/rotation pool. `CheckInCreate.self_role_choice` (`'standard' | 'ncs' | 'logger'`) gates the actual grant, and is **fail-safe**: it defaults to `'standard'` in the schema and the grant tests membership in `('ncs', 'logger')`, so only an affirmative "Check in as NCS"/"Check in as Logger" request grants anything — an omitted field, an explicit `null`, or any caller that simply forgets all check in as a participant. This field was a bool (`check_in_as_standard`) until 2026-09-05; before that, it defaulted to `False` until 2026-09-03, which made *omitting* the field a request for NCS and left correctness resting on ~7 scattered frontend literals staying right — never reintroduce a falsy/omittable default for either grantable role. `NetResponse.current_user_ncs_eligible` / `current_user_logger_eligible` (same two helpers) drive whether each choice is even offered, unaffected by anyone else's current role status. They can also step down after the fact via the existing Acting as NCS/Standard toggle (`nets_roles.py::toggle_self_net_role`, requires an existing NCS role — cannot be used to gain NCS for the first time), which now actually changes backend permission thanks to the `is_active` fix above. `Claim NCS` (owner/admin-only recovery for orphaned nets) is intentionally unchanged/still narrow — the only recovery path on ad hoc nets. History: gated 2026-09-02 after the ME Dirigo Net incident (net 79, 2026-08-30, an off-week rotation member silently auto-granted NCS with no choice presented) by also blocking eligibility whenever anyone else was already active NCS; that block broke net 15's multi-desk pattern and crashed `GET /nets/{id}` outright for every net with 2+ active NCS rows (`scalar_one_or_none()` on a non-unique query, 18 nets on production) — removed 2026-09-03, since the actual incident is fully covered by the two default-to-Standard fixes above. `self_checkin_enabled=False` on a net (only NCS/Logger/owner/admin may submit *any* check-in, per `check_net_permission` in the gate at the top of `create_check_in`) used to block this self-grant path entirely for anyone not already holding one of those roles — a deadlock, since the whole point of self-grant is to let an eligible person become Logger/NCS *by* checking themselves in. Fixed 2026-09-09 (net 88: W1WNS, an eligible co-manager, got 403'd repeatedly and had to be manually promoted by the owner): the gate now also passes a self-check-in through when `self_role_choice` requests `ncs`/`logger` and the requester is actually eligible per the same two functions the grant itself uses — `matching_user`/`linked_user_id` were hoisted above the gate so it can check `linked_user_id == current_user.id` without a duplicate query. Widened 2026-09-18 (net 95, WSSM MOTA) from "active co-manager or rotation member" to **any active `TemplateStaff`**: that schedule had 3 active staff, no co-manager and no rotation, so its auto-created net had no `NetRole` at all (`_assign_duty_ncs` only draws from the rotation), its lobby auto-opened unstaffed, and none of the 3 could take net control — an admin had to use `Claim NCS`. Every net from that schedule since auto-create landed (46, 70, 95 — Jul/Aug/Sep) needed the same rescue. **The regression was not in `permissions.py`**: commit `563ccf5` (2026-06-18) replaced "a human clicks Create Net and `create_net_from_template` assigns them NCS" with the background auto-create job, and in that same commit gave plain active staff the `start_net` bypass and `can_manage=True` — so the frontend has offered them management controls ever since while `check_net_permission` (NetRole-only) refused them. `e76f482` (rotation-only duty assign) and `f1356ac` (self-grant for co-managers/rotation) each patched an adjacent case and left the plain-staff-no-rotation shape uncovered. Real-data check before shipping: 288 pairs evaluated, 0 exceptions; 70 newly-eligible pairs on production, only 4 on a still-open net (net 61's SKYWARN draft, its own staff). Both lookups in the helper are `.limit(1)` existence checks because `ncs_rotation_members` has **no** uniqueness constraint on `(template_id, user_id)` — `scalar_one_or_none()` there is the same latent `MultipleResultsFound` class as the 2026-09-03 outage (0 duplicate rows on production today, so this is preventive). Regression tests: `test_eligible_rotation_member_can_self_grant_logger_when_self_checkin_disabled`, `test_non_eligible_user_still_blocked_when_self_checkin_disabled`, `test_plain_active_staff_can_self_grant_ncs`, `test_plain_active_staff_can_self_grant_logger`, `test_inactive_staff_is_not_eligible`, `test_plain_staff_entered_checkin_still_grants_nothing`, `test_eligibility_tolerates_duplicate_rotation_rows` in `backend/tests/test_ncs_auto_grant_gate.py` |
