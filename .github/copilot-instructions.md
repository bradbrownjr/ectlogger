...
| Backup NCS auto-grant | `backend/app/routers/check_ins.py::create_check_in` (auto-grant block, after the auto-opened-lobby check); `permissions.py::is_eligible_for_ncs_auto_grant` (shared eligibility check) | An active co-manager (`TemplateStaff.is_co_manager`) or active `NCSRotationMember` for a net's template is auto-assigned a `NetRole("NCS")` the moment they check *themselves* into any occurrence, even off-week — no separate "claim" step. Restricted to self-check-in (`current_user.id == linked_user_id`); a staff-entered check-in (NCS/Logger logging someone in, e.g. by voice) never auto-grants, always Standard. Scoped to template-based nets; ad hoc nets have no co-manager/rotation pool. `CheckInCreate.check_in_as_standard` lets an eligible self-check-in opt out up front; `NetResponse.current_user_ncs_eligible` (same helper) drives the "Check in as NCS / Check in as Participant" choice on both the check-in prompt Snackbar and `CheckInFormDialog.tsx`. They can also step down after the fact via the existing Acting as NCS/Standard toggle (`nets_roles.py::toggle_self_net_role`), which now actually changes backend permission thanks to the `is_active` fix above. `Claim NCS` (owner/admin-only recovery for orphaned nets) is intentionally unchanged/still narrow — redundant for this pool since they're auto-granted, but still the only recovery path on ad hoc nets |
