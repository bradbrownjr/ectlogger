...
- Server-originated types (a route handler calls `manager.broadcast` after a DB write): `status_change`, `check_in_deleted`, `hand_raised_changed`, `net_started`, `net_lobby_opened`, `net_status_change`, `net_pause_change`, `role_change`, `can_hear_changed`, `chat_message`, `chat_reaction`, `chat_image`, `traffic_logged`, `ping`
...
| WebSocket real-time updates | `backend/app/main.py::ConnectionManager`; `frontend/src/hooks/useNetWebSocket.ts` | Server-originated: `status_change`, `check_in_deleted`, `hand_raised_changed`, `net_started`/`net_lobby_opened`, `net_status_change`, `net_pause_change`, `role_change`, `can_hear_changed`, `chat_message`/`chat_reaction`/`chat_image`, `traffic_logged`, `ping`. Client-relayed: `check_in`, `active_speaker`, `active_frequency` |
