...
| Auto-close net after inactivity | `backend/app/ncs_reminder_service.py::_check_and_close_inactive_nets`/`_find_auto_close_candidates`/`_last_activity_at`; `backend/app/services/net_closure.py::close_net_and_notify`; `frontend/src/components/create-net/BasicInfoTab.tsx`; `frontend/src/components/create-schedule/ScheduleTab.tsx` | `auto_close_after_minutes` on both `NetTemplate` and `Net` (null = off, the default; 15-1440 range enforced in `schemas.py`, UI only offers 60/120/180/240). Scoped to `NetStatus.ACTIVE` only (never LOBBY). Last activity = max of the net's `started_at`, every `CheckIn.updated_at`/`checked_in_at`, and every `ChatMessage.created_at` -- check-ins and chat both reset the clock, so a quiet-but-monitored net is never closed out from under someone. On threshold breach, posts a custom system chat message ("Net automatically closed after N hour(s) of inactivity.") then calls `close_net_and_notify(..., post_chat_message=False)` so the closure email/ICS-309 looks exactly like a normal manual close -- the auto-close reason lives only in the chat message, never in the email. History: added 2026-09-15 after net 92 (ME Dirigo Net) was left open for hours when NCS forgot to click Close; off by default since a long SKYWARN/ARES activation can legitimately go hours between check-ins. Tests: `backend/tests/test_net_auto_close.py` |
