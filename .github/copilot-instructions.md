...
| Backup NCS auto-grant | `backend/app/routers/check_ins.py::create_check_in` (auto-grant block, after the auto-opened-lobby check); `permissions.py::is_eligible_for_ncs_auto_grant` (shared eligibility check) | An active co-manager (`TemplateStaff.is_co_manager`) or active `NCSRotationMember` for a net's template is auto-assigned a `NetRole("NCS")` the moment they check *themselves* into any occurrence, even off-week — no separate "claim" step — but **only when no one else already holds an active NCS role on that net**. Restricted to self-check-in (`current_user.id == linked_user_id`); a staff-entered check-in (NCS/Logger logging someone in, e.g. by voice) never auto-grants, always Standard. Scoped to template-based nets; ad hoc nets have no co-manager/rotation pool. `CheckInCreate.check_in_as_standard` lets an eligible self-check-in opt out up front; `NetResponse.current_user_ncs_eligible` (same helper) drives the "Check in as NCS / Check in as Participant" choice on both the check-in prompt Snackbar and `CheckInFormDialog.tsx` — because eligibility itself is now gated on there being no active NCS yet, that choice simply stops appearing once someone's already running the net, rather than asking a question with no real answer. `CheckInFormDialog`'s NCS/Standard radio (and `NetView.tsx::handleOpenCheckIn`, which opens it) default to Standard, not NCS -- becoming NCS must be an affirmative click, never an unattended default. They can also step down after the fact via the existing Acting as NCS/Standard toggle (`nets_roles.py::toggle_self_net_role`), which now actually changes backend permission thanks to the `is_active` fix above. `Claim NCS` (owner/admin-only recovery for orphaned nets) is intentionally unchanged/still narrow — redundant for this pool since they're auto-granted, but still the only recovery path on ad hoc nets. Gated 2026-09-02 after the ME Dirigo Net incident (net 79, 2026-08-30): an off-week rotation member self-checking in as a participant while the actual NCS was already running the net got silently auto-granted NCS too, which then showed up as a second, wrong "Net Control Station" in the report |
